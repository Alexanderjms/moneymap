...
export default function GastosClient() {
  ...
  const upsertGasto = useUpsertGasto();
  ...
  const {
    ...
    open,
    close,
  } = useDrawer("gasto-form", "/api/gastos");
  ...
  function openEdit(gas: Gasto) {
    setName(gas.name);
    setAmount(Number(gas.amount).toLocaleString("en"));
    setCurrency(gas.currency || "HNL");
    setDate(gas.date);
    open({
      id: gas.id,
      name: gas.name,
  ...
  return (
    <>
      ...
      <ConfirmModal
        onConfirm={(id) => deleteGasto.mutate(id)}
      />
          ...
          <form id="gasto-form" onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <input
                ...
                className="w-full rounded-lg border border-neutral-700 bg-neutral-800/50 px-3 py-2.5 text-sm text-white outline-none transition-colors focus:border-red-500 focus:ring-1 focus:ring-red-500"
              />
            </div>
            <div className="flex gap-3">
              <div className="flex-1">
                ...
                <div className="flex items-center rounded-lg border border-neutral-700 bg-neutral-800/50 text-sm transition-colors focus-within:border-red-500 focus-within:ring-1 focus-within:ring-red-500">
            ...
            <button
              type="submit"
              disabled={upsertGasto.isPending}
              className="w-full cursor-pointer rounded-md bg-red-700 px-4 py-2.5 text-sm font-medium text-white transition-colors hover:bg-red-800 disabled:opacity-50"
            >
              {upsertGasto.isPending ? "Guardando..." : "Guardar"}
            </button>
          </form>
        </div>
      </div>
